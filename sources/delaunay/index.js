...
(function(window, document) {
    ...
    function documentClick(e) { 
        ...
        imageIndex = (imageIndex + 1) % IMG_PRESETS.length;
        generateStep(IMG_PRESETS[imageIndex]);
    }
    ...
    function documentDrop(e) {
        ...
        if (!window.FileReader) {
            alert('file operation unavaliable');
            return;
        }
    ...
    function generateStep(step) {
        ...
        if (step == 'source') {
            ...
            // normalize every pixel's red value as their lightness value
            Filter.grayscaleFilterR(imageData);
            if (step == 'grayscale') {
                context.putImageData(imageData, 0, 0)
                return
            }
            // blur: average those abnormally high points
            Filter.convolutionFilterR(blur, imageData, blur.length);
            if (step == 'blur') {
                context.putImageData(imageData, 0, 0)
                return
            ...
            // get all the points which has a red value greater than EDGE_DETECT_VALUE
            var temp = getEdgePoint(imageData);
            ...
            // randomly extract points from all the detected points
            // && POINT_MAX_NUM at most 
            while (i < limit && i < ilen) {
                j = tlen * Math.random() | 0;
                points.push(temp[j]);
                temp.splice(j, 1);
            ...
            if (step == 'points') {
                var p, pl = points.length;
                context.clearRect(0, 0, width, height);
                context.fillStyle = 'rgb(255, 255, 255)';
                for (p = 0; p < pl; p++) {
                    context.rect(points[p][0], points[p][1], 1, 1);
                }
                context.fill();
